...
### Edad de la muestra
```{r}
summary(df_form$edad)
boxplot(df_form$edad, ylab = "Edad")
```

### Género
```{r}
plot(edad~genero, data=df_form)
```
...
plot(edad~ingreso, data=df_form, horizontal = TRUE, las = 2, xlab="")
...
plot(edad~distrito, data = df_form, horizontal=TRUE, las=2, xlab="")
title(ylab="Distrito", line=8)
```

### Nivel educativo
```{r}
plot(edad~estudio, data = df_form)
```

### Empleo
```{r}
plot(edad~empleo, data = df_form)
```

### Frecuencia de salidas
```{r}
plot(edad~salidas, data = df_form)
```
