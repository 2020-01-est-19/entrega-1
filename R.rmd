...
df_form <- read_csv(tmp)
```

### Edad de la muestra
```{r}
summary(df_form$"¿Cuál es tu edad?")
```

### Género
```{r}
df_form$"¿De qué género eres?" %>% table() %>% barplot()
```
...
### Distrito
```{r}
par(mai = c(0.75, 2, 0.25, 1))
df_form$"¿En qué distrito vives?" %>% table() %>% barplot(horiz = TRUE, las = 1)
```
