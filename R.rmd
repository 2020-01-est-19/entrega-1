...
$$U.Bound => 34.57 + 1.96 * 12.61/sqrt(n) = 35.97$$
$$L.Bound   => 34.57 - 1.96 * 12.619/sqrt(n) = 33.17$$
...
* H0: Mayores casos durante la medida de inmovilización social no implica necesariamente un aumento de la movilización social.
...
* H0: Menores movilizaciones durante la medida Aprobación del Padrón de Hogares generó más infectados
...
* H0: Las personas con menores frecuencias de salida no supone necesariamente que tengan más ingresos mensuales
...
El tamaño de la muestra adecuado para el nivel de confianza esperado se obtiene a partir de:
...
|---|----------|------|
| Edad |[0,75] | 39.8| u ± 1.47|
| Frecuencia de salida por semana |[0,5]   | 2.5| u ± 0.18|
...
* H0: Las personas con menores frecuencias de salida  necesariamente no supone que tengan más ingresos mensuales
...
Al parecer los puntos de arriba a la izquierda representan los días anteriores
a la cuarentena.
La mancha de la izquierda a los días normales al inicio de la cuarentena.
Finalmente, los puntos de abajo a la izquierda a los días domingos sin movilización.
...
```{r}
plot(df_trabajo$salidas~df_trabajo$empleo, ylab="Frecuencia de salida promedio", xlab=" Variable dicotomizada Tipo de trabajo" )
```


Se observa una relación entre el tipo de trabajo formal y la frecuencia de salida promedio, parece ser que una mayor frecuencia de salida esta asociada a un empleo de tipo formal. Para corroborar ello, se analiza el coeficiente de correlación biserial puntual.

## Pruebas de Hipótesis:

* H0: Las personas con un tipo de trabajo formal no supone que tengan una mayor frecuencia de salida promedio.

H1: Las personas con un tipo de trabajo formal supone que tengan una mayor frecuencia de salida promedio.
...
* H0: Las personas con un tipo de trabajo no supone que tengan una disminución en la frecuencia de salida.

* H1: Las personas con un tipo de trabajo no supone que tengan una disminución en la frecuencia de salida.
...
* H0: El retiro de los 2000 soles de las personas afiliadas al fondo AFP no supone que tengan una disminución en la frecuencia de salida.

* H1: El retiro de los 2000 soles de las personas afiliadas al fondo AFP supone que tengan una disminución en la frecuencia de salida.
...
* H0: Personas del género femenino no suponen mayor frecuencia de salida a la semana.

* H1: Personas del género femenino suponen mayor frecuencia de salida a la semana.
...
* H0: Personas del género masculino no suponen mayor frecuencia de salida a la semana.

* H1: Personas del género masculino suponen mayor frecuencia de salida a la semana.
...
* H0: Existe una correlación entre la edad y la frecuencia de salida.

* H1: No existe una correlación entre la edad y la frecuencia de salida.
...
edad_frec
plot(datos.n~df_form2.edad,data = edad_frec, xlab="edad", ylab="frecuencia")
...
* H0: Mayores ingresos no suponen mayor frecuencia de salida a la semana.

* H1: Mayores ingresos suponen mayor frecuencia de salida a la semana.
...
df_ing_freq <- data.frame(x, y)
plot(y~x, df_ing_freq)

lm_ing_freq <- lm(y~x, df_ing_freq)
abline(lm_ing_freq)
