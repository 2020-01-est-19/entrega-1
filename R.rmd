...
hist(df_form2$edad, xlab = "Edad")
```

### Género
```{r}
plot(edad~genero, data=df_form2)
```
...
plot(edad~ingreso, data=df_form2, horizontal = TRUE, las = 2, xlab="")
...
### Nivel educativo
```{r}
plot(edad~estudio, data = df_form2)
```

### Empleo
```{r}
plot(edad~empleo, data = df_form2)
```

### Frecuencia de salidas
```{r}
summary(df_form2$salidas)
plot(edad~salidas, data = df_form2)
```

### Personas por frecuencia de salida
```{r}
df_form2 %>%
    group_by(salidas) %>%
    summarise(n=n()) %>%
    barplot(n~salidas, data=., ylab = "Personas", xlab="Salidas por semana")
```
...
```{r}
ggplot(aes(y = distrito, x = ingreso), data = df_5) + geom_boxplot()
```

### Distrito vs empleo
```{r}
ggplot(aes(y = distrito, x = empleo), data = df_5) + geom_boxplot()
```
...
df_form2$decceleration %>%
    ...
    table() %>%
    sort(TRUE) %>%
    knitr::kable(col.names = c("Medida", "Cantidad"))
```
...
df_form2$acceleration %>%
    ...
    table() %>%
    sort(TRUE) %>%
    knitr::kable(col.names = c("Factor", "Cantidad"))
```
