...
Hay que limpiar los datos antes de hacer esto.

### Distrito
```{r}
par(mai = c(0.8, 2, 0.2, 0.5))

boxplot(edad~distrito, data = df_form, horizontal=TRUE, las=2, ylab="")
```

### Nivel educativo
```{r}
boxplot(edad~estudio, data = df_form)
```

### Empleo
```{r}
boxplot(edad~empleo, data = df_form)
```

### Frecuencia de salidas
```{r}
boxplot(edad~salidas, data = df_form)
```
